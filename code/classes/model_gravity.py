...
import numpy as np
from agent_grav import Gang, GangMemberG
from helpers import *
import random
import time
...
class GangRivalry(Model):
    ...
    def __init__(self, observed_graph, all_graph, boundaries, road_density, areas, xmax=100, ymax=100, min_jump=0.1,
        ...
        for gang in gang_info.values():
            gang_sizes.append(gang.size)
        self.min_gang = min(gang_sizes)
        self.max_gang = max(gang_sizes)
        print(self.min_gang, self.max_gang)
        self.gang_info = gang_info
        self.norm_gang_size = [(i - self.min_gang)/(self.max_gang - self.min_gang) for i in gang_sizes]
        self.gang_info = gang_info
    ...
    def new_agent(self, pos, name):
        ...
        """
        x, y = pos
        agent = GangMemberG(self.next_id(), self, pos, name,self.min_jump, self.weight_home, self.bounded_pareto, self.kappa, self.vision, self.beta)
...
if __name__ == "__main__":
    ...
    # print(model.areas[(401, 142)])
    # print(list(model.areas.keys()))
    
        riv_mat = model.run_model(step_count=2000000)
        data = model.datacollector.get_model_vars_dataframe()
        np.save('../results/gravity/gr'+ str(i) + '.npy', riv_mat)
        data.to_csv('../results/gravity/w' + str(i) + '.csv')
        print(data)
    # start = time.time()
